...
public class ThresholdFilter implements Filter
{
  ...
  {
    if ((data == null)) throw new SizeException(); // Spec 4.1

    ArrayList<LabeledDouble> result = new ArrayList<LabeledDouble>(); // Spec 4.2
    if (sign == null || sign.length == 0) return result; // Spec 4.2.2
    ...
    for (LabeledDouble x: data)
    {
      ...
      {
        if (comparison == sign[i]) result.add(x); // Spec 4.2.1; 4.2.3; 4.2.4
      }
    }
    return result;
  }
